...
#Football Rest
Rest_ECG_Football <- Football %>% select(ECG_Rest_3,ECG_Rest_4,
                                    ECG_Rest_5,ECG_Rest_6)
...
###### Polar reliability
#Recreational Rest
Rest_Polar_Recreational <- Recreational %>% select(Polar_Rest_3,
...
#Football Rest
Rest_Polar_Football <- Football %>% select(Polar_Rest_3,Polar_Rest_4,
                                           Polar_Rest_5,Polar_Rest_6)
...
#####Apple Watch 6
#Recreational Rest
Rest_Apple6_Recreational <- Recreational %>% select(Apple6_Rest_3,Apple6_Rest_4,
                                       Apple6_Rest_5,Apple6_Rest_6) 
...
## BLAND ALTMAN basic
BA_ECGvsPolar_Post_Recreational <- bland.altman.plot(ECGvsPolar_Post_Recreational$ECG_Post,
                                                     ECGvsPolar_Post_Recreational$Polar_Post,
                                        graph.sys="ggplot2") + theme_classic()
...
## BLAND ALTMAN basic
BA_ECGvsPolar_Post_Football <- bland.altman.plot(ECGvsPolar_Post_Football$ECG_Post,
                                                 ECGvsPolar_Post_Football$Polar_Post,
                                                     graph.sys="ggplot2") + theme_classic()
...
BA_ECGvsAPPLE6_Moderate_Football <- bland.altman.plot(ECGvsAPPLE6_Moderate_Football$ECG_Moderate,
                                                      ECGvsAPPLE6_Moderate_Football$APPLE6_Moderate,
                                                          graph.sys="ggplot2") + theme_classic()
...
Cor_ECGvsAPPLE6_High_Recreational <- ECGvsAPPLE6_High_Recreational %>% 
  ggplot(aes(x=Apple6_High, y=ECG_High)) + geom_point() + theme_bw()
Cor_ECGvsAPPLE6_High_Recreational

#ICC
icc(ECGvsAPPLE6_High_Recreational, model = "twoway", type = "consistency", unit = "average")

## BLAND ALTMAN basic
BA_ECGvsAPPLE6_High_Recreational <- bland.altman.plot(ECGvsAPPLE6_High_Recreational$ECG_High,
                                                      ECGvsAPPLE6_High_Recreational$Apple6_High,
                                        graph.sys="ggplot2") + theme_classic()
...
BA_ECGvsAPPLE6_High_Football <- bland.altman.plot(ECGvsAPPLE6_High_Football$ECG_High,
                                                  ECGvsAPPLE6_High_Football$Apple6_High,
                                                      graph.sys="ggplot2") + theme_classic()
...
## BLAND ALTMAN basic
BA_ECGvsAPPLE7_Low_Recreational <- bland.altman.plot(ECGvsAPPLE7_Low_Recreational$ECG_Low,
                                                     ECGvsAPPLE7_Low_Recreational$APPLE7_Low,
                                        graph.sys="ggplot2") + theme_classic()
...
annotate_figure(BA_recreational,      
                top = text_grob("          At Rest                               Walking                                Jogging                                Running                                   Recovery", 
...
## Data subsets Reliability and Valiidty plotsfor recreational and football athletes
ICC_Reliability <- read_excel("Apple_Data_Condensed_Final_All.xlsx", 
                           sheet = "ICC_Reliability")


ICC_Reliability_Recreational <- ICC_Reliability %>% 
  filter(Group=="Recreational") %>% arrange() %>% 
  mutate(Stage=factor(Stage, levels=c("Rest","Low", "Moderate", "High" ,"Post"))) %>% 
  ggplot(aes(x=Stage, y=ICC, group=Device, color=Device)) +
  ggtitle("Recreational Athletes Reliability") +
  geom_point(aes(shape = Device),size=4.0) + ylim(0.89,1.0) + 
  geom_line(size=0.5) +theme_bw() 


ICC_Reliability_Football <- ICC_Reliability %>% 
  filter(Group=="Football") %>% arrange() %>% 
  mutate(Stage=factor(Stage, levels=c("Rest","Low", "Moderate", "High" ,"Post"))) %>% 
  ggplot(aes(x=Stage, y=ICC, group=Device, color=Device)) +
  ggtitle("Football Athletes Reliability") +
  geom_point(aes(shape = Device),size=4.0) + ylim(0.85,1.0) + 
  geom_line(size=0.5) +theme_bw() 
  

ICC_Reliability_Plot <- ggarrange(ICC_Reliability_Recreational,ICC_Reliability_Football, 
                          ncol = 1,
                          labels = c("A","B"),
                          common.legend = TRUE,legend="bottom")
ggsave("ICC_Reliability_Plot.png")

ICC_Validity <- read_excel("Apple_Data_Condensed_Final_All.xlsx", 
                       sheet = "ICC_Validity")

ICC_Validity_Recreational <- ICC_Validity %>% 
  filter(Group=="Recreational") %>% arrange() %>% 
  mutate(Stage=factor(Stage, levels=c("Rest","Low", "Moderate", "High" ,"Post"))) %>% 
  ggplot(aes(x=Stage, y=ICC, group=Device, color=Device)) +
  ggtitle("Recreational Athletes Reliability") +
  geom_point(aes(shape = Device),size=4.0) + ylim(0.83,1.0) + 
  geom_line(size=0.5) +theme_bw() 

ICC_Validity_Football <- ICC_Validity %>% 
  filter(Group=="Football") %>% arrange() %>% 
  mutate(Stage=factor(Stage, levels=c("Rest","Low", "Moderate", "High" ,"Post"))) %>% 
  ggplot(aes(x=Stage, y=ICC, group=Device, color=Device)) +
  ggtitle("Football Athletes Reliability") +
  geom_point(aes(shape = Device),size=4.0) + ylim(0.49,1.0) + 
  geom_line(size=0.5) +theme_bw() 

ICC_Validity_Plot <- ggarrange(ICC_Validity_Recreational,ICC_Validity_Football, 
                                  ncol = 1,
                                  labels = c("A","B"),
                               common.legend = TRUE,legend="bottom")
ggsave("ICC_Validity_Plot.png")
